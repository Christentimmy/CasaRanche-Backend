...
export const validatePostContent = (
  ...
  const violenceCount = VIOLENCE_KEYWORDS.filter((word) =>
  ...
  const nsfwCount = NSFW_KEYWORDS.filter((word) =>
    lowerText.includes(word)
  ).length;
  ...
  const drugCount = DRUG_KEYWORDS.filter((word) =>
    lowerText.includes(word)
  ).length;
...
export const processMentionsInText = (text: string, mentionedUsers: IUser[]) => {
  ...
  mentionedUsers.forEach((user) => {
    ...
    const replacement = `<span class="mention" data-user-id="${user._id}">@${user.username}</span>`;
